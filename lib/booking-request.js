...
export const normalizeBookingRequest = (input, {
  ...
  const bookingOpensAt = parsedDate.startOf('day').subtract(BOOKING_LEAD_DAYS, 'day').hour(8)
...
export const getBookingSchedule = (request) => {
  ...
  const openingDay = targetDate.subtract(BOOKING_LEAD_DAYS, 'day')
  const bookingOpensAt = openingDay.hour(8).minute(0).second(0).millisecond(0)
  const scheduleAt = openingDay.hour(PREPARATION_HOUR).minute(PREPARATION_MINUTE).second(0).millisecond(0)
  return {
